...
export function useChessGame({
  ...
  const resetGame = useCallback(async () => {
    ...
    // We might want to clear images too? No, keep images.
    // Re-initialize piece instances for the new board
    const newInstances: Record<string, PieceInstance> = {};
    freshGame.board().forEach(row => {
      row.forEach(piece => {
        if (piece) {
          const id = `${piece.color}_${piece.type}_${piece.square}`;
          // Try to reuse existing personalities if available, otherwise default
          const existing = Object.values(pieceInstances).find(p => p?.type === piece.type && p?.color === piece.color);
          
          const personality: PiecePersonality = existing?.personality || {
            names: [`${piece.color === 'w' ? 'White' : 'Black'} ${pieceTypeMap[piece.type]}`],
            description: "A stoic chess piece.",
            voice: piece.color === 'w' ? 'Kore' : 'Puck',
            voicePrompt: "Speak clearly.",
          };
          
          newInstances[piece.square] = { 
            id, 
            name: personality.names[0], 
            personality, 
            ...piece 
          };
        }
    ...
    setPieceInstances(newInstances);
  }, [pieceInstances]);
