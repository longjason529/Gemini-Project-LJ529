...
const PieceCard: React.FC<PieceCardProps> = ({ 
  ...
  isTransient, onGenerateImage, onGenerateBio, isGeneratingBio, onImageTransformChange, onPersonalityChange, onNameChange,
            ...
            {!isEditingPersonality && piece.color === 'w' && (
              <div className="section-actions" style={{ display: 'flex', gap: '0.5rem' }}>
                <button 
                  ...
                  className="edit-btn" 
                  aria-label="Generate new bio" 
                  disabled={isGeneratingBio}
                  title="Generate AI Personality"
                >
                  <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>
                    {isGeneratingBio ? 'progress_activity' : 'auto_awesome'}
                  </span>
                </button>
