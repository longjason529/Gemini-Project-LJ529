...
import { GrandmasterChat } from './components/GrandmasterChat';
...
function App() {
  ...
  const [showGrandmasterChat, setShowGrandmasterChat] = useState(false);
        ...
        <div className="left-panel">
              <div className="header-actions">
                {SHOW_DEV_CONTROLS && (
                  <>
                    <button 
                      ...
                      onClick={() => {
                        setShowStrategyConsole(!showStrategyConsole);
                        if (showGrandmasterChat) setShowGrandmasterChat(false);
                      }}
                ...
                <button 
                  ...
                  onClick={() => {
                    if (window.confirm("Start a new game? All current progress will be lost.")) {
                      chessGame.resetGame();
                      interactions.handleSquareClick(null as any);
                    }
                  }}
                  aria-label="New game"
                  title="New Game"
                ...
                <button 
                  className="icon-btn"
                  onClick={() => {
                    // Automate a move for the user
                    const { bestMove } = findBestMove(chessGame.game, 3);
                    if (bestMove) {
                      chessGame.executeMove(bestMove);
                    }
                  }}
                  aria-label="Automate move"
                  title="Automate Best Move"
                >
                  <span className="material-symbols-outlined">magic_button</span>
                </button>
                <button 
                  className={`icon-btn ${showGrandmasterChat ? 'active' : ''}`}
                  onClick={() => {
                    setShowGrandmasterChat(!showGrandmasterChat);
                    if (showStrategyConsole) setShowStrategyConsole(false);
                    if (showCodeAssistant) setShowCodeAssistant(false);
                  }}
                  aria-label="Toggle Grandmaster chat"
                  title="Grandmaster Advisor"
                >
                  <span className="material-symbols-outlined">psychology</span>
                </button>
                <button 
                  className={`icon-btn ${showCodeAssistant ? 'active' : ''}`}
                  onClick={() => {
            ...
            {showStrategyConsole ? (
              ...
              />
               ...
               <GrandmasterChat 
                 game={chessGame.game} 
                 onClose={() => setShowGrandmasterChat(false)} 
               />
            ) : showCodeAssistant ? (
              <CodeAssistant onClose={() => setShowCodeAssistant(false)} />
            ) : (
              pieceToDisplay ? (
                <PieceCard
                  ...
                  onGenerateBio={() => customization.generateBio(pieceToDisplay!, chessGame.setPieceInstances, interactions.setChattingWith)}
                  isGeneratingBio={customization.generatingBioId === pieceToDisplay.id}
